...
class TestSorting:
    ...
    async def test_sortable_columns_exist(self):
        ...
        for expected in ["Pod Name", "CPU Usage", "Memory Usage"]:
            assert any(
                expected in col for col in sortable_columns
            ), f"Expected column '{expected}' not found"
        ...
        if table_data:
            ...
            if len(numeric_values) > 1:
                ...
                if sort_direction == "desc":
                    assert numeric_values == sorted(
                        numeric_values, reverse=True
                    ), "CPU usage not sorted in descending order"
                else:
                    assert numeric_values == sorted(
                        numeric_values
                    ), "CPU usage not sorted in ascending order"
    ...
    async def test_sort_memory_usage(self):
        ...
        if table_data:
            ...
            memory_usage_col = 8
            memory_values = [
                row[memory_usage_col] for row in table_data if len(row) > memory_usage_col
            ]
            ...
            if len(numeric_values) > 1:
                if sort_direction == "desc":
                    assert numeric_values == sorted(
                        numeric_values, reverse=True
                    ), "Memory usage not sorted in descending order"
                else:
                    assert numeric_values == sorted(
                        numeric_values
                    ), "Memory usage not sorted in ascending order"
    ...
    async def test_sort_toggle_direction(self):
        ...
        ), "Sort direction should toggle when clicking the same header"

        # Should toggle between asc and desc
        assert {first_direction, second_direction} == {
            "asc",
            "desc",
        }, "Sort should toggle between ascending and descending"
    ...
    async def test_sort_different_columns(self):
        ...
        assert (
            pod_name_direction_after == "none"
        ), "Previous sort column should not show sort indicator"
